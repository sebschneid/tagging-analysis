...
class ExtractionConfig(pydantic.BaseModel):
    ...
    file_suffix: str
    start: Optional[str] = "00:00"
    stop: Optional[str] = "120:00"
    halftime: Optional[str] = "60:00"
...
class PackingCount(pydantic.BaseModel):
    ...
    @pydantic.validator("count_to_value")
...
class AggregationRow(pydantic.BaseModel):
    ...
    def create_from_config(cls, extraction_config: ExtractionConfig):
        ...
        (
            ...
            filter_time=True,
            seconds_start=tagging.helpers.string_time_to_seconds(
                extraction_config.start
            ),
            seconds_stop=tagging.helpers.string_time_to_seconds(
                extraction_config.stop
            ),
        )
...
def extract_rows(aggregation_rows: List[AggregationRow]) -> pd.DataFrame:
    ...
    for row in aggregation_rows:
        ...
        aggregation_series.append(packing_series)
